...
palabras_negativas = {'wasnt', 'mightnt', 'couldnt', 'shant', 'mustnt', 'werent', 'wouldnt', 'arent', 'neednt', 'dont', 'doesnt',
            'isnt', 'wont', 'didnt', 'hadnt', 'havent', 'hasnt', 'shouldnt', 'thatll', 'youll', 'its','im', 'yours', 'shes', 'hes',
                      'thats', 'whats', 'youd', 'youve', 'ours', 'yourselves', 'himself', 'herself', 'theirs', 'his', 'was', 'is', 'i'}
...
df = pd.read_csv('C:\\Users\\Oscar Tello\\Desktop\\programas_datos\\Datos\\tweets\\UKRAINE.csv')
...
corpus = st.CorpusWithoutCategoriesFromParsedDocuments(
    df, parsed_col='parse'
).build().get_unigram_corpus().remove_infrequent_words(minimum_term_count=30)
...
open("Dispersion_UKRAINE.html", 'wb').write(html.encode('utf-8'))
